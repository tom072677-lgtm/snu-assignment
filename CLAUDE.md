...
- **정치외교학부(`political_science`):** 구 도메인 `polisci.snu.ac.kr` NXDOMAIN. 신 도메인 `psir.snu.ac.kr`는 AJAX 게시판(POST `/event/listProc` → JSON). `gnbType=01&type=01` 필수. 상세 URL `/event/${pid}` (path 방식). `headerlist`(14개 고정 공지) + `list`(10개 일반) 합산 = 24건. `_server` + `httpPost`/`jsonList` (서버: `deptNotices.js`). 중복 방지: URL + title+date 이중 dedup.
