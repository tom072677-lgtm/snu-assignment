...
- **정치외교학부(`political_science`):** 구 도메인 `polisci.snu.ac.kr` NXDOMAIN. 신 도메인 `psir.snu.ac.kr`는 AJAX 게시판(POST `/event/listProc` → JSON). `gnbType=01&type=01` 필수 — 없으면 테스트 board 반환. 상세 URL은 `/event/${pid}` (path 방식). `_server` + `httpPost`/`jsonList`로 수정(기기 검증 완료, 10건 실제 2026 공지). 서버: `deptNotices.js`.
